import librosa
from transformers import Wav2Vec2ForCTC, Wav2Vec2Processor, Wav2Vec2Processor
...
class DetectStuttering:
    ...
    def transcribe_detect_stuttering(self):
        ...
        # If proper transcript is not provided, generate it from the target_transcript method
        ...
        # Decode predicted IDs to obtain the transcription
        predicted_ids = torch.argmax(logits, dim=-1)
        transcription = tokenizer.batch_decode(predicted_ids)[0]

        # Find indices where the transcription differs from the proper transcript
        stuttering_indices = []
        for i, (char, proper_char) in enumerate(zip(transcription, self.proper_transcript)):
            if char != proper_char:
                stuttering_indices.append(i)

        # Group stuttering indices into segments
        stuttering_segments = []
        ...
        # Find sequences of characters not in common between transcription and proper transcript
        stuttered_chars = self.find_sequences_not_in_common(transcription, self.proper_transcript)

        # Calculate total number of mismatched characters
        total_mismatched_chars = sum(len(segment) for segment in stuttered_chars)

        # Calculate normalized mismatch percentage between 0 and 100
        proper_transcript_length = len(self.proper_transcript)
        normalized_mismatch_percentage = (total_mismatched_chars / proper_transcript_length) * 100

        # Normalize the value between 0 and 100
        max_possible_mismatch = len(self.proper_transcript)
        normalized_mismatch_percentage = min(normalized_mismatch_percentage, max_possible_mismatch)
        normalized_mismatch_percentage = (normalized_mismatch_percentage / max_possible_mismatch) * 100
        normalized_mismatch_percentage = round(normalized_mismatch_percentage)

        # Prepare the full result text
        full_text = ""
        full_text += "Normalized Mismatch Percentage: " + str(normalized_mismatch_percentage) + "%" + "\n"
        full_text += "Mismatched Chars: ["
        ...
        full_text += "Proper Transcript: " + self.proper_transcript + "\n"
    ...
    def stuttering_seconds(self) -> tuple[float, list]:
        if self.proper_transcript == "":
            self.proper_transcript = self.target_transcript().upper()
        else:
            self.proper_transcript = self.proper_transcript.upper()
    ...
    def predict_megamgem(self):
        concatenated_vector_get = Functions(self.audio_file).get_concatenated_vector_get()
        prediction = Files("src/classification_model.h5").predict_with_model(concatenated_vector_get)
